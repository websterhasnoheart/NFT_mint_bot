...
from Account_Settings.accounts import AccountExt
from config.settings import w3, CHAIN_ID, DEFAULT_GAS, logger, LOGGING
...
def send_many_to_many(senders_accounts: List[AccountExt],
    ...
    gas_price = w3.eth.gas_price
    tx_fee = DEFAULT_GAS * gas_price
    available_balances = {sender_account.address: old_balances[sender_account.address] - tx_fee for sender_account in
                          senders_accounts}

    for sender_account, receiver_account in zip(senders_accounts, receivers_accounts):
        nonce = sender_account.get_nonce()
        try:
            last_tx_hash = send_tx(sender_account, receiver_account, available_balances[sender_account.address],
                                   gas_price, nonce)
            if LOGGING == True:
