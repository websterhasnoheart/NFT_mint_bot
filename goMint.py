"""
======================= READ THIS YOU MFERS =======================
How the FUCK should I use this? I dont know about Python..

ANS:
Go to your command line, windows + R -> type in 'cmd' and type in the following message, just one line!
usage: goMint.py [-h] [-single | -multi | -newacc]
...
Example: python goMint.py -multi -> You mint NFT for multiple accounts
"""

from xml.etree.ElementTree import TreeBuilder

from black import main
from Account_Settings.accounts import get_master_account, get_derived_accounts, display_accounts
from Account_Settings.splitter import send_many_to_many, send_one_to_many, send_many_to_one
from Account_Settings.interactions import contract_write, contract_write_from_one
from Account_Settings.utils import estimate_single_mint_fee, estimate_multi_mint_fees
from config.settings import w3, MINT_FUNCTION_NAME, MINT_PRICE, NUMBER_OF_MINTS, EXTRA_MIXING_LAYERS,\
    LOGGING, CONTRACT_ADDRESS, CHAIN_NAME, SEND_BACK, FEES_MULT_FACTOR, MINT_DATE, logger, parser
import time
...
if __name__ == '__main__':
    while True:
        time.sleep(1)
        mint_time = datetime.strptime(MINT_DATE,"%H:%M:%S")
        time_now = datetime.strptime(datetime.now().time().strftime("%H:%M:%S"), "%H:%M:%S")
        time_left = mint_time - time_now
        print(time_left)
        if datetime.now().time().strftime("%H:%M:%S") == MINT_DATE:
            print('It is time, LFG!')
            break
    ...
    master_account = get_master_account(defult=True)
    master_account_balance = master_account.get_balance()
    print('======================= Mint is starting =======================')
    ...
    if input('Type y for yes to RUN the program') in ['y', 'Y']:
      print('Program running')
      logger.info('======================= LETS GO FUCKING MINT THIS THING =======================')
